...
@section('title', __('website.admin.projects.create_title') . ' - JCI Carthage')
...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold jci-primary-text">{{ __('website.admin.projects.create_title') }}</h1>
    </div>
    ...
    <div class="jci-card p-6">
        <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <p class="mt-1 text-sm text-gray-500">{{ __('website.admin.projects.image_hint') }}</p>
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <select name="icon_color" id="icon_color"
                    ...
                    <option value="blue" {{ old('icon_color', 'blue') == 'blue' ? 'selected' : '' }}>{{ __('website.admin.events.color_blue') }}</option>
                    <option value="yellow" {{ old('icon_color') == 'yellow' ? 'selected' : '' }}>{{ __('website.admin.events.color_yellow') }}</option>
                    <option value="green" {{ old('icon_color') == 'green' ? 'selected' : '' }}>{{ __('website.admin.events.color_green') }}</option>
                    <option value="purple" {{ old('icon_color') == 'purple' ? 'selected' : '' }}>{{ __('website.admin.events.color_purple') }}</option>
                    <option value="red" {{ old('icon_color') == 'red' ? 'selected' : '' }}>{{ __('website.admin.events.color_red') }}</option>
                </select>
                @error('icon_color')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6 space-y-2">
                ...
                <label class="flex items-center">
                    <input type="checkbox" name="show_on_website" value="1" {{ old('show_on_website', true) ? 'checked' : '' }}
                           class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">

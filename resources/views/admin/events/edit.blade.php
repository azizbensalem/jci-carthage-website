...
@section('title', 'Modifier un événement - JCI Carthage')
...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold jci-primary-text">Modifier l'événement</h1>
    </div>
        ...
        <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="4" required
                       class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $event->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="mb-3">
                    ...
                    <img src="{{ Storage::url($event->image) }}" alt="Image actuelle" class="max-w-xs rounded-lg shadow-md mb-2">
                </div>
                @endif
            ...
            <div class="mb-4">
                ...
                <input type="text" name="icon" id="icon" value="{{ old('icon', $event->icon) }}" placeholder="M12 4.354a4 4 0 110 5.292..."
            ...
            <div class="mb-4">
                ...
                <select name="icon_color" id="icon_color"
                    ...
                    <option value="blue" {{ old('icon_color', $event->icon_color) == 'blue' ? 'selected' : '' }}>Bleu</option>
                    <option value="yellow" {{ old('icon_color', $event->icon_color) == 'yellow' ? 'selected' : '' }}>Jaune</option>
                    <option value="green" {{ old('icon_color', $event->icon_color) == 'green' ? 'selected' : '' }}>Vert</option>
                    <option value="purple" {{ old('icon_color', $event->icon_color) == 'purple' ? 'selected' : '' }}>Violet</option>
                    <option value="red" {{ old('icon_color', $event->icon_color) == 'red' ? 'selected' : '' }}>Rouge</option>
                </select>
                @error('icon_color')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6 space-y-2">
                ...
                <label class="flex items-center">
                    <input type="checkbox" name="show_on_website" value="1" {{ old('show_on_website', $event->show_on_website) ? 'checked' : '' }}
                           class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                    <span class="ml-2 text-sm text-gray-700">Afficher sur le site web</span>
                </label>
            </div>

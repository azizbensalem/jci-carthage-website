...
@section('title', __('website.admin.presidents.edit_title') . ' - JCI Carthage')
...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold jci-primary-text">{{ __('website.admin.presidents.edit_title') }}</h1>
    </div>

    <div class="jci-card p-6">
        <form action="{{ route('admin.presidents.update', $president) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-700 mb-2">{{ __('website.admin.presidents.name_field') }}</label>
                <input type="text" name="name" id="name" value="{{ old('name', $president->name) }}" required
            ...
            <div class="mb-4">
                ...
                <input type="text" name="presidency_year" id="presidency_year" value="{{ old('presidency_year', $president->presidency_year) }}" required
            ...
            <div class="mb-4">
                ...
                <div class="mb-3">
                    <p class="text-sm text-gray-600 mb-2">{{ __('website.admin.presidents.current_photo') }}</p>
                    <img src="{{ $president->photo_url }}" alt="{{ $president->name }}" class="h-28 w-28 rounded-2xl object-cover shadow-md">
                </div>
                @endif
                <input type="file" name="photo" id="photo" accept="image/jpeg,image/png,image/jpg,image/gif,image/webp"
                ...
                <p class="mt-1 text-sm text-gray-500">{{ __('website.admin.presidents.photo_hint_edit') }}</p>
                @error('photo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <div id="photoPreview" class="mt-4 hidden">
                    <p class="text-sm text-gray-600 mb-2">{{ __('website.admin.presidents.preview') }}</p>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="order" id="order" value="{{ old('order', $president->order) }}" min="0"
            ...
            <div class="mb-6">
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $president->is_active) ? 'checked' : '' }}
